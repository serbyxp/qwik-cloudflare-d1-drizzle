import type { EventId, Group, GroupId, UserSession } from "~/database/";
import { getEvent, getMembership, getGroupById, getDB } from "~/database";
import { AuthenticationError, NotFoundError } from "~/utils";
...
export async function hasAccessToGroup(
  ...
  const membership = await getMembership(user.id, groupId, db);
...
export async function assertEventExists(eventId: EventId) {
