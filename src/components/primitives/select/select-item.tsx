...
export const HSelectItem = component$<SelectItemProps>((props) => {
  ...
  useTask$(async function getIndexTask() {
    if (_index === undefined)
      throw Error(
        "Qwik UI: Select component item cannot find its proper index."
      );
  ...
  useTask$(async function navigationTask({ track, cleanup }) {
    ...
    const hasScrollbar =
      context.popoverRef.value.scrollHeight >
      context.popoverRef.value.clientHeight;
  ...
  const handleKeyDown$ = $(async (e: KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case "Home":
        if (context.isListboxOpenSig.value) {
          context.highlightedIndexSig.value =
            await getNextEnabledItemIndex$(-1);
        }
        break;
