...
export const HPopoverPanelImpl = component$<PropsOf<"div">>((props) => {
  ...
  if (hasRenderedOnClientSig.value === 1) {
    // Now run the task again after we force-rendered the context
    setTimeout(() => {
  ...
  useTask$(async ({ track, cleanup }) => {
    ...
    if (context.panelRef?.value) {
      const topLayerAncestor = await findTopLayerAncestor$(
        context.panelRef.value
      );
  ...
  return (
    <div
      ...
      id={panelId}
      ref={panelRef}
      popover={context.manual || props.popover === "manual" ? "manual" : "auto"}
      onBeforeToggle$={[
        $(async (e) => {
          if (!context.panelRef?.value) return;

          if (e.newState === "open") {
            await supportShowAnimation(
              context.panelRef.value,
              isPolyfillSig.value
            );
          }
      ...
      onToggle$={[
        $((e: CorrectedToggleEvent) => {
          ...
          if (
            ...
            context.panelRef.value.parentElement.appendChild(
              context.panelRef.value
            );
          }
        }),
