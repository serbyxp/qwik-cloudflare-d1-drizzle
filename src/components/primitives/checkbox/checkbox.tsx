...
import type { TriBool } from "../checklist/checklist-context-wrapper";
...
export const ChecklistTwoStateCheckbox =
  component$<ChecklistTwoStateCheckboxProps>((props) => {
    ...
    const syncToChecklist = useSignal<undefined | boolean>(
      props._overWriteCheckbox,
    );
    useContextProvider(CheckboxContext, appliedSig);
    useTask$(({ track }) => {
...
export const MixedStateCheckbox = component$<MixedStateCheckboxProps>(
  (props) => {
    console.log("mixed");
...
const TwoStateCheckboxBehavior = component$<TwoStateCheckboxBehaviorProps>(
  (props) => {
    const handleKeyDownSync$ = sync$((e: KeyboardEvent) => {
      if (e.key === " ") {
        e.preventDefault();
